...
@section('content')
    ...
    <div class="page-content fade-in-up">
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">List of Sub-Categories</div>
            </div>
            <div class="ibox-body">
            ...
            @endif
              ...
              @if(\App\Helper\CustomHelper::canView('Create Sub-Category', 'Super Admin|Cashier'))
            <div class="row">
              @if(session()->has('success'))
                <p class="alert alert-success" role="alert">{!! session()->get('success') !!}</p>
                @endif
                <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                  <a href="{{ route('admin.sub-categories.create') }}" class="btn btn-success"> <i class="fa fa-plus"></i>New Sub-Category</a>
                </div>
              </div>
              @endif
                   ...
                   <thead>
                    ...
                    @foreach($showData as $key => $val)
                     <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                        ...
                        <td class="p-1 text-capitalize">{{ $val->subcategory_parent?->name }}</td>
                        {{-- <td class="p-1 text-capitalize"></td> --}}
                        ...
                        <td width="200" class="p-1">{{ $val->updated_by }}</td>
                          ...
                          <td class="text-center p-1" width="100">
                              <a href="{{ route('admin.sub-categories.manage', $val->id) }}" class="btn btn-success"> <i
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.sub-categories.destroy') }}",
          method: "delete",
          dataType: "html",
          data: {id: pid},
