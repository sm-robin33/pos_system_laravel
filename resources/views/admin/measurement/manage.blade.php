...
<div class="page-content">
  <div class="ibox">
      <div class="ibox-head">
          <div class="ibox-title">Manage Measurement</div>
      </div>
      <div class="ibox-body">
          <div class="row">
              <div class="col-md-12">
                @if(\App\Helper\CustomHelper::canView('List of Measurement', 'Super Admin|Cashier'))
                <div class="row">
                      ...
                      <div class="form-row">
                        ...
                        <input type="hidden" name="id" value="{{ $measurements->id }}">
                          <label for="full_name" class="col-form-label">Measurement Name:</label>
                            <input type="text" name="name" placeholder="Measurement Name" required value="{{ $measurements->name }}"
                          ...
                          <label for="status" class="col-form-label">Status:</label>
                            <select name="status" required class="form-control @error('status') is-invalid @enderror">
                              ...
                              @foreach(\App\Models\Measurement::$statusArrays as $status)
                                <option value="{{ $status }}"
                                        @if(old('status',$measurements->status) == $status) selected @endif>{{ ucfirst($status) }}</option>
                              @endforeach
                            </select>
                            @error('status')
