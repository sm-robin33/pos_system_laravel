...
@section('content')
    ...
    <div class="page-content fade-in-up">
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">List of Inventories</div>
            </div>
            <div class="ibox-body">
            ...
            @endif
              ...
              @if(\App\Helper\CustomHelper::canView('Create Inventory', 'Super Admin|Cashier'))
            <div class="row">
              @if(session()->has('success'))
                <p class="alert alert-success" role="alert">{!! session()->get('success') !!}</p>
                @endif
                <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                  <a href="{{ route('admin.inventory.create') }}" class="btn btn-success"> <i class="fa fa-plus"></i>New Inventory</a>
                </div>
              </div>
              @endif
                   ...
                   <thead>
                    ...
                    @foreach($showData as $key => $val)
                     <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                        ...
                        <td class="p-1 text-capitalize">{{ $val->category_parent?->name }}</td>
                        <td class="p-1 text-capitalize">{{ $val->category_parent?->name }}</td>
                        <td class="p-1 text-capitalize">{{ $val->category_parent?->name }}</td>
                        <td class="p-1 text-capitalize">{{ $val->category_parent?->name }}</td>
                        <td class="p-1 text-capitalize">{{ $val->category_parent?->name }}</td>
