...
@section('content')
    <div class="page-heading">
        <h1 class="page-title">Discount Policy List</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{route('admin.dashboard')}}"><i class="fa fa-home font-20"></i></a>
            </li>
            <li class="breadcrumb-item">Discount Policy List</li>
        </ol>
    </div>
    <div class="page-content fade-in-up">
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">List of Discount Policy</div>
            </div>
            <div class="ibox-body">
            ...
            @endif
              ...
              @if(\App\Helper\CustomHelper::canView('Create Discount', 'Super Admin|Cashier'))
            <div class="row">
              @if(session()->has('success'))
                <p class="alert alert-success" role="alert">{!! session()->get('success') !!}</p>
                @endif
                <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                  <a href="{{ route('admin.discount.create') }}" class="btn btn-success"> <i class="fa fa-plus"></i>New Discount Policy</a>
                </div>
              </div>
              @endif
                   ...
                   <thead>
                    ...
                    @foreach($showData as $key => $val)
                     <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                        ...
                        <td class="p-1 text-capitalize">{{ $val->policy_name }}</td>
                        <td class="p-1 text-capitalize">{{ $val->disc_amount }}</td>
                        <td class="p-1 text-capitalize">{{ $val->start_date }}</td>
                        <td class="p-1 text-capitalize">{{ $val->end_date }}</td>
                        ...
                        <td width="200" class="p-1">{{ $val->updated_by }}</td>
                          ...
                          <td class="text-center p-1" width="100">
                              <a href="{{ route('admin.discount.manage', $val->id) }}" class="btn btn-success"> <i
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <strong>Are you sure to delete this policy?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.discount.destroy') }}",
          method: "delete",
          dataType: "html",
          data: {id: pid},
