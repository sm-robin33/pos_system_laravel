...
<div class="page-content">
  <div class="ibox">
      <div class="ibox-head">
          <div class="ibox-title">Manage Customer</div>
      </div>
      <div class="ibox-body">
          <div class="row">
              <div class="col-md-12">
                @if(\App\Helper\CustomHelper::canView('List of Cudtomer', 'Super Admin|Cashier'))
                <div class="row">
                  ...
                  <form action="{{ route('admin.customer.store') }}" method="post" enctype="multipart/form-data">
                      ...
                      <div class="form-row">
                        ...
                        <input type="hidden" name="id" value="{{ $customers->id }}">
                          <label for="full_name" class="col-form-label">Full name</label>
                            <input type="text" name="name" placeholder="Full Name" required value="{{ $customers->name }}"
                     ...
                     @enderror
                        ...
                        <label for="phone_no" class="col-form-label">Phone No</label>
                          <input type="number" name="phone" placeholder="Phone No" value="{{ $customers->phone }}"
                   ...
                   @enderror
                        ...
                        <div class="col-md-5 mt-2">
                          <label for="email" class="col-form-label">Email</label>
                            <input type="email" name="email" placeholder="Email" value="{{ $customers->email }}"
                     ...
                     @enderror
                        ...
                        <label for="address" class="col-form-label">Address</label>
                          <input type="text" name="address" placeholder="Address" value="{{ $customers->address }}"
                        ...
                        <label for="gender" class=" col-form-label">Gender</label>
                          <select name="gender" required class="form-control @error('gender') is-invalid @enderror">
                            ...
                            @foreach(\App\Models\Customer::$genderArrays as $gender)
                              <option value="{{ $gender }}"
                                      @if(old('gender',$customers->gender) == $gender) selected @endif>{{ ucfirst($gender) }}</option>
                            @endforeach
                          </select>
                          @error('gender')
                          ...
                          <label for="status" class=" col-form-label">Status</label>
                            <select name="status" required class="form-control @error('status') is-invalid @enderror">
                              ...
                              @foreach(\App\Models\Customer::$statusArrays as $status)
                                <option value="{{ $status }}"
                                        @if(old('status',$customers->status) == $status) selected @endif>{{ ucfirst($status) }}</option>
                              @endforeach
                            </select>
                            @error('status')
