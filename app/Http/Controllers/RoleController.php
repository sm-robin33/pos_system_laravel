...
class RoleController extends Controller
{
  ...
  {
    if (in_array($id, [1,2])){
      return RedirectHelper::routeWarning('admin.role.list', '<strong>Sorry!!!</strong> Role update not possible');
    }
    if ($data['role'] = Role::find($id)) {
      return view('admin.permission.role.manage', $data);
    }
    return RedirectHelper::routeWarning('admin.role.list', '<strong>Sorry!!!</strong> Role not found');
  }
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $role->guard_name = 'web';
      if ($role->save()) {
        return RedirectHelper::routeSuccess('admin.role.list', $message);
      }
